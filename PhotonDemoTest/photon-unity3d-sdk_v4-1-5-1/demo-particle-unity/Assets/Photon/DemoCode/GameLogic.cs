    ...
    public class GameLogic : LoadBalancingClient
    {
        ...
        private void SendPositionUpdate()
        {
            if (this.UseInterestGroups)
            {
                ...
                this.LoadBalancingPeer.OpRaiseEvent(DemoConstants.EvPosition, this.LocalPlayer.WriteEvMove(), new RaiseEventOptions() { InterestGroup = playerGroup }, new SendOptions() { Reliability = this.SendReliable });
            }
            else
            {
                // this overload of OpRaiseEvent sends to everyone in the room - even those who didn't subscribe to any group
                this.LoadBalancingPeer.OpRaiseEvent(DemoConstants.EvPosition, this.LocalPlayer.WriteEvMove(), new RaiseEventOptions(), new SendOptions() { Reliability = this.SendReliable });
            }
        }
